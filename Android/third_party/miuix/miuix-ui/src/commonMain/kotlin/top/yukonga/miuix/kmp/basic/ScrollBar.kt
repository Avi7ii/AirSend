 ...
 * @param alwaysVisible Keep the scroll position visible when the component is idle.
 */
@ExperimentalScrollBarApi
@Composable
...
private fun ScrollBar(
    ...
    var opacity by remember(alwaysVisible) {
        mutableFloatStateOf(if (alwaysVisible) 1f else 0f)
    }
    ...
    LaunchedEffect(isHighlighted, alwaysVisible) {
        if (alwaysVisible) {
            hideJob?.cancel()
            opacity = 1f
    ...
    LaunchedEffect(adapter, alwaysVisible) {
        snapshotFlow { adapter.scrollOffset }
            .drop(1)
            .collect {
                opacity = 1f
                if (!isHighlighted && !alwaysVisible) {
                    hideJob?.cancel()
                    hideJob = coroutineScope.launch {
                        delay(ScrollBarDefaults.FadeDelayMillis.toLong())
                        animate(
