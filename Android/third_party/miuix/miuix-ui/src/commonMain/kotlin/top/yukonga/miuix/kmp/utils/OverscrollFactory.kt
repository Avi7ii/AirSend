...
class MiuixOverscrollEffect(
    private val settleDelayMillis: Long = 0L,
    private val topEdgeInertiaEnabled: Boolean = true,
) : OverscrollEffect {
    ...
    private fun topEdgeAdjustedVelocity(velocity: Float): Float =
        if (!topEdgeInertiaEnabled && offsetY >= 0f && velocity > 0f) 0f else velocity
    ...
    override suspend fun applyToFling(
        ...
        if (!bypassY && isActiveY && velocity.y != 0f) {
            if (sign(velocity.y) == sign(offsetY)) {
                ...
                startSpringAnimationY(topEdgeAdjustedVelocity(velocity.y))
        ...
        if (!bypassY) {
            startSpringAnimationY(topEdgeAdjustedVelocity(remaining.y / 1.53333f))
        }
